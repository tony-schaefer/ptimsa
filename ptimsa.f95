      program ptimsa
!     Program to Insert Molecules in Solvated Areas
!     last updated 10-10-16
!     input files are coordinates in pdb or gro format
!     proper gromacs installation is recommended to make use of gmx make_ndx
...
! grab file extension
      ...
      ! goto 97
      !endif
! i want it to work without being provided a topology file, but don't remember why
! the only reason why the top file is useful it to keep molecule parameters
...
! append each atom number to the list if it is in the specified group
...
! if extension is not gro, assume pdb
        boxatoms=0
        do
...
! use current time to be more randumb
        ...
        randumb=rand()
        randumb=rand()
! the 1st randumb number is randumb for my compiler, always low
...
! to get them farther apart
                ...
                call sqdist(tryxyz(l,:),tryxyz(l,:),box,c)
                ...
                if(abs(psi).gt.1.2.or.psi.eq.0.) then
...
! rotate it
              ...
              if(attempt.ge.20) then
                write(1,*) "adjusting didn't help much, consider -dl ",&
                  molres(o)
                exit outer
              endif
            enddo
